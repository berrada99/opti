...
def multistart(mymodel, iter, gen_multi, localsolver, labels,
               logfile = None, infofile = None, i = 0, real_R = 0, epsilon = 10**-4):
...
def MBH(mymodel, gen, localsolver, labels,
    ...
    if check_if_optimal(results):
        ...
        infofile.write("No feasible solution\n")

    if feasible == True:
        print(algo_name + " Best record found  {0:8.8f}".format(best_obj))
        infofile.write("{0} {1:8.8f} ".format(str(i), best_obj))
        err = abs((best_obj - real_R)*100)
        if err > 0.1:
            infofile.write("N          ")
        else:
            infofile.write("Y          ")
        infofile.write("{0:4.4f}     ".format(err))
        LoadPoint(mymodel, bestpoint)
        printPointFromModel(mymodel)
    else:
        print(algo_name + " No feasible solution found by local solver")
